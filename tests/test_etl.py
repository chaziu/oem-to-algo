import unittest
import pandas as pd
from etl import Compare, Df, Data_process
...
class TestCompare(unittest.TestCase):
    ...
    def test_find_delta_records(self):
        ...
        with self.subTest():
            self.assertTrue(to_create == [{'ref_no': '2019CPHIK-0073', 'Company': 'JAPAN No.2 ltd', 'Country': 'JAPAN'}], 'Records to create has error')

        with self.subTest():
            self.assertTrue(to_update == [{'ref_no': '2019CPHIK-0071', 'Company': 'SEOUL No.1 ltd', 'Country': 'KOREA', 'objectID':1003}], 'Records to update has error')

        with self.subTest():
            self.assertTrue(to_delete == [{'ref_no': '2019CPHIK-0069', 'Company': 'Taiwan No.1 ltd', 'Country': 'Taiwan', 'objectID':1001}], 'Records to delete has error')
